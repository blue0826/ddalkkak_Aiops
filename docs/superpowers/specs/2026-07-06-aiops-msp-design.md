...
> **상태**: §10 결정 완료 (4·5번만 보류) — ⏸ **P0 착수 대기** (CEO 토큰 갱신 후 "진행" 지시 트리거)
...
3. ~~첫 타깃 클라우드 순서~~ ✅ **확정**: 추천 채택 + **SCP가 메인** — 설계는 동시, 실측·우선순위 SCP 우선
4. **Central 호스팅 위치** — ⏸ 보류 ("상황에 따라") — P0 실측 결과 보고 결정
5. **에디션·가격 구조** — ⏸ 미정 — P0 이후 CFO 시뮬레이션으로 재상정
6. ~~완전 에어갭 고객 실존 여부~~ ✅ **확정**: 고객 실존. 단 시스템 검증 후 대응 (P3 로드맵 유지, 선행 투자 없음)
7. ~~ITSM~~ ✅ **확정**: 추천 채택 — P2까지 자체 경량 모듈
8. ~~LLM 공급~~ ✅ **확정**: 개발 단계는 Claude Code CLI 로그인(구독) 방식. 운영 단계 공급은 추후
9. ~~Collector 언어~~ ✅ **확정**: 추천 채택 — Python 시작, P3 전 Go 전환 검토

**실행 지침 (2026-07-06 CEO)**: 코딩은 전부 서브에이전트 디스패치, **토큰 비싼
모델 금지** (워커 = Sonnet/Haiku). P0 착수는 CEO 토큰 갱신 후 "진행" 지시 대기.
